...
import com.shadentcg.shop.model.OrderItem;
    ...
    private SalesReport generateReport(String type, LocalDate start, LocalDate end) {
        List<Order> orders = orderRepository.findAll().stream()
            .filter(o -> {
                ...
                return !placed.isBefore(start) && !placed.isAfter(end) && o.getStatus() != Order.OrderStatus.CANCELLED;
            })
            .toList();
        ...
        return new SalesReport(type, start, end, totalRevenue, orders.size(), totalUnits, topProducts);
    }
}
